...
import type { ServerMessage, PreComputedState } from './ws-protocol.js';
import type {
  WorkerReplayPayload,
  WorkerToMainMessage,
} from './types.js';
 ...
 *
 * `replay-precompute.ts` also emits `WORKER_REPLAY_BOARD_STATES` and
 * `WORKER_REPLAY_COMPLETE` — included here so the emitter covers every
 * worker-to-main variant, even though replay-precompute holds its own
 * `replayDuelId` variable (the factory is called with that id).
 */
...
export interface WorkerEmitter {
  ...
  /** WORKER_REPLAY_BOARD_STATES — precomputed timeline chunk for replay viewer. */
  replayBoardStates(turnNumber: number, states: PreComputedState[]): void;
...
export function createWorkerEmitter(port: TypedPort, duelId: string): WorkerEmitter {
  return {
    ...
    replayBoardStates:  (turnNumber, states) => port.postMessage({ type: 'WORKER_REPLAY_BOARD_STATES', duelId, turnNumber, states }),
