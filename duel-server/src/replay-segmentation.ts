...
export const SEGMENTATION: Readonly<Record<ServerMessage['type'], SegmentationAction>> = {
  ...
  // MSG_WIN terminates the duel but is treated as a normal visual event
  // here : the precompute appends it to the current timeline entry's
  // events array (label is generated by the front via the MSG_WIN row in
  // the journal, see `DuelGameLogService`). The state that carries
  // MSG_WIN is then flushed by the final-state emit at the end of
  // `runReplayPreComputation`. Replay-precompute does NOT use it as a
  // flush boundary.
  MSG_WIN: 'accumulate',
