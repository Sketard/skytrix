...
import { loadOppTurnEnablers } from './latent-interruption-computer.js';
...
console.log('\n🔬 Test D.6: Masquerena in M3 + another monster in BOTH EMZs → slot unavailable');
{
  const state = makeFieldState({
    M3:    [makeCard(MASQUERENA)],
    EMZ_L: [makeCard(APOLLOUSA)],
    EMZ_R: [makeCard(APOLLOUSA)],
    EXTRA:  [makeCard(KNIGHTMARE_PHOENIX, 'facedown')],
  });
  const result = scorer.score(state);
  // Masquerena is on M3 (active), but both EMZs are occupied by others — slot unavailable.
  // V1 does not check MZONE-via-arrow.
  assert(result.scoreBreakdown.latentPoints === 0,
    `latentPoints=${result.scoreBreakdown.latentPoints} (expected 0 — both EMZ occupied, no arrow path V1)`);
}
