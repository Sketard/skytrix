...
- CLAUDE.md "Pre-activation Buffer (defense in depth, post-`ANIMATIONS_READY`)" :
  section ré-écrite post-Direction B.
- CLAUDE.md "Modes — vue d'ensemble" : le SOLO multiplex shortcut
  `derivePhase` reste en place (gating le flash dice).

### Review Findings (2026-06-05, commit `661d6458`)

Code-review Winston via `bmad-code-review` — 3 reviewers parallèles
(Blind Hunter, Edge Case Hunter, Acceptance Auditor). 29 findings
collectés, 14 après dédup + classification.

- [x] [Review][Decision] **F1 — `startRematch` bypasse le gate `animationsReady` → race SOLO rematch réintroduite** [`duel-server/src/session-orchestrator.ts:226-263`] — `resetSessionForRematch` reset `animationsReady=[false,false]` mais la ligne suivante spawn `startDuelWithOrder` (SOLO) ou `startFirstPlayerPhase` (PvP) sans consulter `isReadyToStart`. Race SOLO bootstrap exactement comme pre-protocole sur chaque rematch. Confirmé par les 3 reviewers (blind+edge+auditor). Reproduit le prior P0 #1 de la Direction A. Options : (a) hoist le spawn dans `onAnimationsReady` quand `phase === 'WAITING_PLAYERS'` (route rematch via la même gate que fresh connect), (b) `resetSessionForRematch` ramène `phase=WAITING_PLAYERS` et le rematch reconnecte via la gate.
- [x] [Review][Patch] **F2 — `_handleEarlyDeckPrefetch` no-op sur `cardCodes: []` → deadlock perma WAITING_PLAYERS** [`duel-server/src/pvp-connection-handler.ts:417-426` + `front/src/app/pages/pvp/duel-page/duel-connection.ts:1443-1458`] — Si `extractCardCodesForPlayer` retourne `[]` (deck mal chargé), client guard `if (message.cardCodes?.length)` skip → prefetch effect ne fire jamais → `ANIMATIONS_READY` jamais émis → server stuck. `duelAssert` côté serveur ET flip `thumbnailsReady=true` côté client si `cardCodes: []`.
- [x] [Review][Patch] **F3 — Typedef `EarlyDeckPrefetchMsg` docblock drift "SESSION_TOKEN" vs "SESSION_PHASE"** [`duel-server/src/ws-protocol-system.ts:135-141` + `front/src/app/pages/pvp/duel-ws-system.types.ts:135-141`] — Docblocks disent "IMMEDIATELY after SESSION_TOKEN" mais le code émet après `SESSION_PHASE`. Remplacer dans les 2 typedefs.
- [x] [Review][Patch] **F4 — `_animationsReadySent=true` avant send + pas de reset sur WS drop → silent deadlock** [`front/src/app/pages/pvp/duel-page/duel-loading-effects.service.ts:171-175`] — Flag set avant `safeSend()` qui peut return false silencieusement si WS close entre gate check et send. `_animationsReadySent` stays true → no re-emission. Fix : `sendAnimationsReady` retourne bool + flag conditionnel + reset sur `connectionStatus !== 'connected'`.
- [x] [Review][Patch] **F6 — Fork-solo en phase WAITING_PLAYERS mis-route vers `startDuelWithOrder`** [`duel-server/src/server.ts:342-368`] — Hook check `phase === 'WAITING_PLAYERS'` AVANT le check `forkMode`. Si un futur refactor crée fork session en WAITING_PLAYERS, branche tire `INIT_DUEL` au lieu de `INIT_FORK`. Short-circuit `if (session.forkMode) { ... fork branch ... return; }` en tête du hook.
- [x] [Review][Patch] **F7 — A2 strict validator pas mis à jour pour `ANIMATIONS_READY`** [`duel-server/src/client-message-validator.ts`] — `DuelWebSocketService.sendAnimationsReady` tag `forPlayer:0` en SOLO. Vérifier que A2 reject `forPlayer` en PvP normal sur ce nouveau type (impersonation guard) et accept `forPlayer:0` en SOLO. Pinner par test.
- [x] [Review][Patch] **F8 — `forkConnectionTimeout` pas clear hors-fork branche** [`duel-server/src/server.ts:344-364`] — Seul le branche fork clear `forkConnectionTimeout`. Hoist au top du hook : `if (session.forkConnectionTimeout) { clearTimeout(...); session.forkConnectionTimeout = null; }`.
- [x] [Review][Patch] **F9 — Fork-solo session leak — disconnect avant ANIMATIONS_READY sans cleanup timer** [`duel-server/src/pvp-connection-handler.ts:382-386 + :590`] — `forkConnectionTimeout` clear au connect, mais si user close tab avant ANIMATIONS_READY, `ws.on('close')` `if (session.soloMode) return;` skip cleanup. Worker + session leak. Pour fork-solo : ne pas clear `forkConnectionTimeout` jusqu'à réception de ANIMATIONS_READY (clear dans le branche fork du hook).
- [x] [Review][Patch] **F10 — `bothCardCodes` shipped en SOLO mais jamais consommé client** [`front/src/app/pages/pvp/duel-page/duel-connection.ts:1443-1458`] — Server `EARLY_DECK_PREFETCH` SOLO emet `bothCardCodes`, client `_handleEarlyDeckPrefetch` lit seulement `cardCodes`. Pop-in opponent cards en SOLO multiplex. Consume `bothCardCodes` (preFetch sur union des 2 decks).
- [x] [Review][Patch] **F11 — Effect order SOLO rematch — registration-order dépendance fragile** [`front/src/app/pages/pvp/duel-page/solo-mode-effects.service.ts:52-60` + `duel-loading-effects.service.ts:191-199`] — `solo-mode-effects` set `thumbnailsReady=true`, `duel-loading-effects` set `thumbnailsReady=false`. Ordre actuel safe par registration order mais un refactor casse l'invariant. Retirer le `set(true)` de `solo-mode-effects` (la chain cardCodes → prefetch couvre déjà).
- [x] [Review][Patch] **F12 — Spec subsection citation incorrecte dans orchestrator docblock** [`front/src/app/pages/pvp/duel-page/animation-orchestrator.service.ts:222-223`] — Cite "§2 'Pre-activation buffer côté client : conservé en mode défensif minimal'" mais le titre actuel est "Pre-activation buffer côté client — conservé". Aligner.
- [x] [Review][Patch] **F13 — Spec §7 Liens référence ancien titre CLAUDE.md** [`_bmad-output/planning-artifacts/animations-ready-protocol-2026-06-05.md:522-523`] — Lien dit "Pre-activation Buffer (initial draw breathe beat) : section à mettre à jour". CLAUDE.md a été retitré "Pre-activation Buffer (defense in depth, post-`ANIMATIONS_READY`)". (Note : ce finding sera auto-résolu par cette section même.)
- [x] [Review][Patch] **F14 — `_handleSessionToken` comment block 7 lignes inutile** [`front/src/app/pages/pvp/duel-page/duel-connection.ts:1716-1723`] — Comment explique "removal" qui n'apparaît pas dans le diff (Direction A pivot pré-existait avant ce commit). Future reader Ctrl-F → confusion. Raccourcir / drop.
- [x] [Review][Defer] **État machine `_animationsReadySent × prefetchStarted × thumbnailsReady`** [`duel-loading-effects.service.ts`] — Pas documenté pour les combos (initial-connect, reconnect, rematch, refresh). À adresser quand F4 sera patché (force la doc de l'état machine). Pre-existing — pas un bug ship.
- [x] [Review][Defer] **Buffer doctrine "narrow window" hand-wave** [`CLAUDE.md:1438-1460`] — Le rationale post-protocole pour conserver le buffer est défendable mais pas testé. Instrumenter pour confirmer que le buffer reste à zéro en steady-state, puis envisager suppression. Cleanup post Phase 1 v4.
- [x] [Review][Defer] **Source-string matching pour wire-order tests** [`duel-server/src/pvp-connection-handler.spec.ts:1026-1052`] — Brittle vs refactor mais fonctionne aujourd'hui et le pattern est consistant avec d'autres tests "Option O source-level guard" dans le projet. Remplacer par un test runtime avec mock `sendToPlayer` plus tard.
- [x] [Review][Defer] **JSDoc SOLO `animationsReady[1]` ambiguous** [`duel-server/src/lifecycle-helpers.ts`] — Tighten doc plus tard.
- [x] [Review][Defer] **EARLY vs DECK_PREFETCH sémantique différente non testée** [`duel-connection.spec.ts:1493-1505`] — Edge case low-priority.
- [x] [Review][Defer] **`extractCardCodesForPlayer` × 2 SOLO** [`pvp-connection-handler.ts:417-424`] — Micro-perf.
- [x] [Review][Defer] **Fork idempotence comment manquant** [`server.ts:366`] — Clarification doc.
- [x] [Review][Defer] **Stale-ws test manquant pour ANIMATIONS_READY** [`duel-server/src/client-message-router.spec.ts`] — Coverage gap mais guard upstream le couvre.
- [x] [Review][Defer] **Fixture `lifecycle-helpers.spec.ts` default `[true,true]` masque** [`duel-server/src/lifecycle-helpers.spec.ts:14-22`] — Pattern acceptable, intent documenté.
