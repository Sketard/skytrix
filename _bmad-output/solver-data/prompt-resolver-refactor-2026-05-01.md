...
| **7. decisionHints population** | 2-3 days | Hand-author the first wave of `decisionHints` for the audited fixtures (snake-eye, branded, ddd, mitsurugi). Each hint backed by empirical sourceCardId verification against the audited baselines. | (revised — see note below) Mechanism validated: each authored hint loads, resolves on the expected `(sourceCardId, promptType)`, fires when DFS visits the prompt, and preserves bit-exact β-1 baselines (the `pendingTargets`-would-match guard makes the hint inert when an explicit plan target is also present). |

**Phase 7 gate revised post-Phase 6 reality (2026-05-01).** The original gate ("DFS-solver standalone reaches β-1 levels on prompt-local decisions, e.g. snake-eye 4/7 via SELECT_YESNO override") was empirically falsified during Phase 7 authoring. The first hint (Divine Temple SELECT_YESNO=yes on snake-eye) loaded and fired correctly on DFS standalone (8+ visits to the prompt, hint applied each time), but `matched` did not move from the 2/7 baseline. Diagnosis: the snake-eye 4/7 trajectory requires a chain of **strategic SELECT_IDLECMD decisions** (NS Ash → Almiraj Link → Activate Divine Temple → Silhouhatte → end-phase). SELECT_IDLECMD is Tier 4 in the Phase 6 coverage matrix ("no source by construction") so cannot be hinted. The hint resolves only **one** prompt (`SELECT_YESNO` mid-combo). The remaining gap between DFS-standalone and β-1 is owned by the scorer/ranker (which prefers a shorter terminal — Silhouhatte+Azurune set+end-phase — over the partial Divine-Temple branch DFS can fit in its 400-node budget). That gap is the territory of Phase B / graph-ml-v2 / scorer redesign, **not** the prompt-resolver refactor. The realistic gate for Phase 7 is therefore mechanism validation across multiple decks: each hint resolves the right prompt, behaves correctly when fired, and never breaks β-1 bit-exact.
